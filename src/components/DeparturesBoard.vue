<template>
  <div class="container flex flex-col items-center">
    <header
      class="bg-amber-400 text-black text-3xl font-semibold flex items-center gap-2 px-8 py-5 shadow-md w-full max-w-5xl"
    >
      <svg
    ...
    async load() {
      ...
      try {
        ...
        this.flights = list.map((f, idx) => ({
          ...
          scheduledTime:
            f.scheduledDepartureDateTime || f.scheduledTime || null,
          flightNumber: f.flightNumber || f.flightCode || "",
          airline: f.airline?.name || f.airline || f.operator || "",
          gate:
            f.departureGate?.number || f.departureGate?.name || f.gate || "",
