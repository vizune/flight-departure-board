<template>
  <form @submit.prevent="onSubmit" class="w-full max-w-5xl mt-6 bg-neutral-900/70 border border-neutral-700 rounded-lg p-6 space-y-4">
    ...
    <div class="grid md:grid-cols-2 gap-4">
      <div>
        <label for="flight" class="block text-sm font-semibold text-gray-300 mb-1">Flight</label>
        <select
          id="flight"
        ...
        >
          ...
          <option
            ...
            :value="f.id"
          >
            {{ f.flightNumber }} — {{ f.arrivalAirport?.cityName || f.arrivalAirport?.airportName || 'Destination' }}
      ...
      <div>
        <label for="statusType" class="block text-sm font-semibold text-gray-300 mb-1">Status</label>
        <select
          id="statusType"
    ...
    <div v-if="form.statusType === 'FreeText'">
      <label for="freeText" class="block text-sm font-medium text-gray-300 mb-1">Custom status</label>
      <input
        id="freeText"
    ...
    <div class="flex items-center gap-3">
      ...
      <p v-if="error" class="text-red-400 text-sm ml-auto" role="alert">{{ error }}</p>
      <p v-else-if="success" class="text-green-400 text-sm ml-auto" aria-live="polite">Status updated.</p>
    </div>
  </form>
</template>
...
export default {
  ...
  computed: {
    canSubmit() {
      ...
      if (this.form.statusType === 'FreeText') return this.form.freeText.trim().length > 0
      return true
    },
  },
  methods: {
